...
The weights of all the nodes are stored in a vector of matrices of size (j x k), where k is the layer size just before the layer with j as its layer size. Initially, the weights are randomly assigned values using the Guassian normal distribution. Though, the weights so get updated once the netwroks starts learning.
	
### biases
	vector<matrix> biases;
The biases of a particular layer in a network are stored in a vector of matrices of size (j x 1), where j is the layer size of that layer. Initially, the biases are randomly assigned values using the Gaussian normal distribution. However, the biases get updates as the network starts learning.
...
The weighted sums of a particular layer in a network are stored in a vector of matrices of size (j x 1), where j is the layer size of that layer.
