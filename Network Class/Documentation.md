...
It takes one parameter, which is the mini_batch_index of datatype `int`.
#### Description:
Once the weighted_sums and the activations matrices get assigned, this method checks for the last activations matrix of the network and compares it with the Expected_values matrix of that mini_batch_index. If they match, it increments the num_correct by 1. 
