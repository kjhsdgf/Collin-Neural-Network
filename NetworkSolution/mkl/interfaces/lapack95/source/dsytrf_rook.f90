...
PURE SUBROUTINE DSYTRF_ROOK_F95(A,UPLO,IPIV,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_SYTRF_ROOK, F77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=10), PARAMETER :: SRNAME = 'SYTRF_ROOK'
    ! <<< Local scalars >>>
    ...
    LWORK = -1
    CALL F77_SYTRF_ROOK(O_UPLO,N,A,LDA,O_IPIV,S_WORK,LWORK,O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_SYTRF_ROOK(O_UPLO,N,A,LDA,O_IPIV,WORK,LWORK,O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
