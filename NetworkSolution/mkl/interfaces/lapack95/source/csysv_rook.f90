...
PURE SUBROUTINE CSYSV_ROOK_F95(A,B,UPLO,IPIV,INFO)
    ...
    ! UPLO='U','L'; default: 'U'
    ! <<< Use statements >>>
    USE F77_LAPACK, ONLY: F77_SYSV_ROOK, F77_XERBLA
    ! <<< Implicit statement >>>
    IMPLICIT NONE
    ! <<< Kind parameter >>>
    ...
    ! <<< Local declarations >>>
    ! <<< Parameters >>>
    CHARACTER(LEN=9), PARAMETER :: SRNAME = 'SYSV_ROOK'
    ! <<< Local scalars >>>
    ...
    LWORK = -1
    CALL F77_SYSV_ROOK(O_UPLO,N,NRHS,A,LDA,O_IPIV,B,LDB,S_WORK,LWORK,   &
     &                                                  O_INFO)
    ...
    IF(L_STAT_ALLOC==0) THEN
        CALL F77_SYSV_ROOK(O_UPLO,N,NRHS,A,LDA,O_IPIV,B,LDB,WORK,LWORK, &
     &                                                      O_INFO)
    ELSE; O_INFO = -1000
    ENDIF
