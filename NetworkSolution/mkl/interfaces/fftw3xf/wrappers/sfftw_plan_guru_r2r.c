...
/*
 *
 * sfftw_plan_guru_r2r - FFTW3 Fortran 77 wrapper to Intel(R) MKL.
 *
 ******************************************************************************
 */
...
void
sfftw_plan_guru_r2r(PLAN *p, INTEGER *rank, INTEGER *n, INTEGER *is,
                    INTEGER *os, INTEGER *howmany_rank, INTEGER *howmany_n,
                    INTEGER *howmany_is, INTEGER *howmany_os, REAL4 *in,
                    REAL4 *out, INTEGER *kind, INTEGER *flags)
{
    fftwf_iodim64 dims64[MKL_MAXRANK];
    fftwf_iodim64 howmany_dims64[MKL_ONE];
    fftwf_r2r_kind knd[MKL_MAXRANK];
    int i;
    ...
    *(MKL_INT64 *)p = 0;
    ...
    for (i = 0; i < *rank; ++i)
    {
        ...
        knd[i] = (fftwf_r2r_kind) kind[*rank - i - 1];
    }
    ...
    *(fftwf_plan *)p =
        fftwf_plan_guru64_r2r(*rank, dims64, *howmany_rank, howmany_dims64, in,
                             out, knd, *flags);
}
