...
/*
 *
 * fftwf_plan_guru_dft_r2c - FFTW3 wrapper to Intel(R) MKL.
 *
 ******************************************************************************
 */
...
/* r2c is Intel(R) MKL DFTI real forward */
static void execute_fi(fftw_mkl_plan p);
static void execute_fo(fftw_mkl_plan p);

fftwf_plan
fftwf_plan_guru64_dft_r2c(int rank, const fftwf_iodim64 *dims, int howmany_rank,
                         ...
                         fftwf_complex *out, unsigned flags)
{
    ...
    if (rank == 1)
        s = DftiCreateDescriptor(&mkl_plan->desc, DFTI_SINGLE, DFTI_REAL,
